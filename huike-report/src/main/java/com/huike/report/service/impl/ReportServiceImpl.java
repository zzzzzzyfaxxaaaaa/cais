...
public class ReportServiceImpl implements IReportService {
     ...
     */
    @Override
    public IndexTodayInfoVO getToday(String format) {
        String username = SecurityUtils.getUsername();
        IndexTodayInfoVO todayInfoVO=new IndexTodayInfoVO();
        todayInfoVO.setTodayCluesNum(reportMpper.getTodayClue(username,format));
        todayInfoVO.setTodayBusinessNum(reportMpper.getTodyBusiness(username,format));
        todayInfoVO.setTodayContractNum(reportMpper.getTodayContract(username,format));
        todayInfoVO.setTodaySalesAmount(reportMpper.getTodaySales(username,format));
        return todayInfoVO;
    }
    ...
    public LineChartVO cluesStatistics(String beginCreateTime, String endCreateTime) {
        ...
        LineChartVO chartVO= null;
        try {
            chartVO = new LineChartVO();
            List<String> allDates=findDates(beginCreateTime,endCreateTime);
            chartVO.setxAxis(allDates);//设置日期集合
            //series
            List<LineSeriesVO> series = new ArrayList<>();
            LineSeriesVO lineSeriesVO1=new LineSeriesVO();//新增线索
            LineSeriesVO lineSeriesVO2=new LineSeriesVO();//线索总数量
            ...
            for (String allDate : allDates) {
                Optional<Map<String, Object>> optional = statistics.stream().filter(d ->
                        d.get("dd").equals(allDate)
                ).findFirst();
                if(optional.isPresent()){
                    Map<String, Object> map = optional.get();
                    lineSeriesVO1.getData().add(map.get("num"));
                    sum+=Integer.parseInt(map.get("num").toString());
                }else {
                    lineSeriesVO2.getData().add(0);
                }
            }
     ...
     */
    @Override
    public List<TargetIndexVo> clueChangeStatisticsForIndex(String beginCreateTime, String endCreateTime) {
        List<TargetIndexVo> list = new ArrayList<>();

        // 数据封装
        list = reportMpper.salesStatistic(beginCreateTime,endCreateTime);

        for (TargetIndexVo indexVo : list) {
            BigDecimal radio = getRadio(indexVo.getNumall(), indexVo.getNum());
            indexVo.setRadio(radio);
            // mybatis驼峰命名法无法直接为Create_by赋值，
            indexVo.setCreate_by(indexVo.getUsername());
        }
        return list;
    }
